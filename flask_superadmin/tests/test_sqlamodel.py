...
def test_model():
    ...
    resp = client.post(
        "/admin/model1/add/", data=dict(test1="test1large", test2="test2")
    )
    ...
    resp = client.post(
        "/admin/model1/%s/delete/" % model.id, data={"confirm_delete": True}
    )
    eq_(resp.status_code, 302)
...
def test_exclude():
    ...
    view = CustomModelView(Model1, db.session, excluded_list_columns=["test2", "test4"])
    admin.add_view(view)
...
def test_search_fields():
    ...
    view = CustomModelView(Model1, db.session, searchable_columns=["test1", "test2"])
    admin.add_view(view)
...
def test_non_int_pk():
    ...
    resp = client.post("/admin/modelview/new/", data=dict(id="test1", test="test2"))
    eq_(resp.status_code, 302)
...
def test_reference_linking():
    ...
    ok_(
        '<input class="" id="name" name="name" type="text" value="Stan">'
        in resp.data.decode()
    )
    ok_(dog_link in resp.data.decode())
...
def test_filters():
    ...
    view = CustomModelView(
        Model1, db.session, list_display=["test1", "test2", "test3", "test4"]
    )
